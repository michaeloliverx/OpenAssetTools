#include "LocalizeDumperIW3Xenon.h"
...
namespace localize
{
    DumperIW3Xenon::DumperIW3Xenon(const AssetPool<IW3Xenon::AssetLocalize::Type>& pool)
        : AbstractSingleProgressAssetDumper(pool)
    {
    }

    size_t DumperIW3Xenon::GetProgressTotalCount() const
    {
        return m_pool.m_asset_lookup.empty() ? 0uz : 1uz;
    }

    void DumperIW3Xenon::Dump(AssetDumpingContext& context)
    {
