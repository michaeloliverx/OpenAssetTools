...
namespace menu
{
    void MenuDumperT5::DumpAsset(AssetDumpingContext& context, const XAssetInfo<AssetMenu::Type>& asset)
    {
        ...
        if (!ObjWriting::ShouldHandleAssetType(ASSET_TYPE_MENULIST))
        {
            ...
            for (auto* menuListAsset : menuListAssets)
                CreateDumpingStateForMenuListT5(zoneState, menuListAsset->Asset());
        }
