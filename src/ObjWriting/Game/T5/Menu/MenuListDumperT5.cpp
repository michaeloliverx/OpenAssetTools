...
namespace
{
    void DumpMenus(menu::IWriterT5& menuDumper, menu::MenuDumpingZoneState* zoneState, const MenuList* menuList)
    {
        if (!menuList->menus)
            return;
...
namespace menu
{
    void CreateDumpingStateForMenuListT5(MenuDumpingZoneState* zoneState, const MenuList* menuList)
    {
    ...
    void MenuListDumperT5::DumpAsset(AssetDumpingContext& context, const XAssetInfo<AssetMenuList::Type>& asset)
    {
        ...
        const auto menuWriter = CreateMenuWriterT5(*assetFile);
